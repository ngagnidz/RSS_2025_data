close all; clc
load('dynamic_data.mat', 'dynamic')
load('static_data.mat', 'static');
x_data = dynamic{:, 2};
y_data = dynamic{:, 3};
z_data = dynamic{:, 4};

[X_grid, Y_grid] = meshgrid(linspace(min(x_data), max(x_data), 100),...
                            linspace(min(y_data), max(y_data), 80));
Z_grid = griddata(x_data, y_data, z_data, X_grid, Y_grid, 'cubic');

s = scatter(x_data, y_data, 3, z_data, 'filled');
s.MarkerFaceAlpha = 0.5;
hold on;
grid on;
errorbar(static.x_centers, static.mean_values, static.ynegdeltas, static.yposdeltas, ...
...
xlabel('Separation, $\Delta z/l$', 'FontSize', 18, 'Interpreter', 'latex');
ylabel('Moment, $\bar{M_y}/(Wl)$', 'FontSize', 18, 'Interpreter', 'latex');
