...
[X_grid, Y_grid] = meshgrid(linspace(min(x_data), max(x_data), 100),linspace(min(y_data), max(y_data), 80));
...
% % Exact same formatting as your reference code
% xlim([0 8]);
% ylim([0.5 1.4]);
% axis("square");

% % Your exact colorbar setup
% a = colorbar;
...
% 
% clim([-0.065, 0.065]);
% scatter([9 9], [0 0], [1 1], [1 -1]);

% xlabel('Separation, $\Delta z/l$', 'FontSize', 16, 'Interpreter', 'latex');
% ylabel('Thrust, $\bar{F_z}/W$', 'FontSize', 16, 'Interpreter', 'latex');
% 
% set(gca, 'FontSize', 14);
% p = gca();
% p.OuterPosition(3) = 0.95;
% figure('Position', [100, 100, 1000, 750], 'Color', 'white');
